...
> 作者：许清楚（产品经理）
> 日期：2026-09-17
> 上游输入：用户拍板的方向转向（停止镜像 Android 数据源链）+ 2026-09-16 实机 HTTP 探测结论（非读文档）+ `PRD-zhisheng-ios-A.md` §1.5/§2.2/§6 + `original-android-README.md` + `feature-parity-zhisheng-ios.md`
> 事实基准：所有端点可用性均以 **2026-09-16 实测状态码**为准；本文档**不修改任何 Swift 源码**，只做需求规划。
> 关系：本文档是 `PRD-zhisheng-ios-A.md` 的**续作**，复用它建立的全部 ID 体系（D1–D37、A1–A3、AC-*）。凡与 PRD-A 冲突处，以本文档 §3「纠偏记录」为准。
...
- **免密的边界已画死**（§2）：免密能拿到**全球网格类**天气/空气/历史/辐射/水文/集合/季节数据；**拿不到**中国分钟级降水、官方分级预警、中国雷达回波——这三项要么上凭据、要么不做，**不再承诺**。
- **本轮三批**：B1 免密拉满（13 项）→ B2 自带 Key 增强层（6 项）→ B3 方向 B 收口（6 项，**在数据面收口之后**才做）。批批独立出 IPA。
- **两处已存在的实现缺陷进入 B1 P0**：星标置顶未生效、异地城市时区未生效（`feature-parity` D-1 / D-4，代码已写未接线）。
- **额度纪律是硬约束**：Open-Meteo 免密 **10,000 次/日**，但**复杂请求按倍数计费**（集合 ×4.0、季节 ×1.3、多模式气候 ×1844.5）；**贵端点必须用户触发或默认关闭**（§4）。
...
| P-4 | 雷达 / 台风 / 预警 / 多源 → 备案不做 | 雷达 / 预警 → 迁入 **B2 凭据层**；台风 → 迁入 **B1**（改用官方 nmc.cn 免密源，标注无 SLA） | 2026-09-16 实测 |
...
| 15 分钟原生覆盖 | 同上（`minutely_15`） | **原生仅美国 / 中国 / 欧洲**；其余地区为**插值** |
| 空气质量 | `air-quality-api.open-meteo.com/v1/air-quality` | `domains=auto\|cams_europe\|cams_global` |
| 历史再分析 | `archive-api.open-meteo.com/v1/archive` | **`start_date`/`end_date` 必填**（无 `past_days`）；ERA5 1940–、ERA5-Land 1950–、IFS 9km 2017–；**ERA5 滞后约 5 天** |
| 地理编码 | `geocoding-api.open-meteo.com/v1/search` | ⚠️ **`/v1/geocoding` 路径不存在**，只有 `/v1/search` |
| 海拔 | `api.open-meteo.com/v1/elevation` | — |
| 海洋 | `marine-api.open-meteo.com/v1/marine` | 仅海域坐标有效 |
| 洪水 | `flood-api.open-meteo.com/v1/flood` | **仅日粒度**：`river_discharge` + `_mean/_median/_max/_min/_p25/_p75`；`forecast_days 0–210`；GloFAS 5km 自 1984 |
| 气候预估 | `climate-api.open-meteo.com/v1/climate` | `models` 必填（7 个 CMIP6 HighResMIP）、仅日粒度、1950–2050；**额度 ×1844.5，本轮不接** |
| 集合预报 | `ensemble-api.open-meteo.com/v1/ensemble` | `models` 必填、`forecast_days 0–35`；**额度 ×4.0** |
| 卫星辐射 | `satellite-api.open-meteo.com/v1/archive` | `models=satellite_radiation_seamless`；GHI/DNI/DHI/GTI；**Himawari-9 覆盖亚洲含中国，0.05° / 10 分钟，2015 起** |
...
| 台风（官方） | `typhoon.nmc.cn/weatherservice/typhoon/jsons/list_default`、`view_{longID}` | **JSONP 双括号包裹，需自定义解析**；120h 预报点 + 30KTS 风圈；**无 SLA、无官方文档** |
...
| 能力 | 提供方 | 代价 | 结论 |
|---|---|---|---|
| 中国分钟级降水（2h / 1km） | 和风 QWeather 免费 key | 1,000 次/日、非商用、**与付费同权** | 推荐 |
| 官方分级预警（蓝/黄/橙/红） | 和风 QWeather | 同上 | 推荐 |
| 官方生活指数 | 和风 QWeather | 同上 | 推荐 |
...
**凭据规范**：`X-QW-Api-Key` 头或 Ed25519 JWT；免费域 `devapi.qweather.com`、付费域 `api.qweather.com`、项目专属域 `xxx.re.qweatherapi.com`。无 key 实测 **403**。
...
| K-3 | §2.2 D5 备注 | 「`minutely_15` 未来约 2h 覆盖」 | ✅ 成立，且**细化**：15 分钟粒度**原生仅美/中/欧**，其余地区为插值 | 落实为 **B1-2**：中国区标「插值」，禁止暗示逐分钟 |
| K-4 | §2.2 D24 备注 | 「中国天气网/彩云瓦片需凭据或地区限定」 | ✅ 成立并**强化**：中国天气网为**主动封禁**（`X-CCDN-FORBID-CODE: 020200`），非仅"需凭据" | 归入 §2.3 档三 |
| K-5 | §2.2 D25 备注 | 台风源「浙江水利厅接口无文档无 SLA」 | ✅ 成立 | **源已更换**：改用官方 `typhoon.nmc.cn`（同样无 SLA，但属官方公开资料）→ **B1-7** |
...
| `archive`（往年同日，5/10 年） | ×1 / 年 | **用户触发** | **限并发 ≤3 + 逐年串行 + 失败年份静默跳过**；不预取 |
| `historical-forecast` | ×1 | 用户触发 | 与 archive 复用同一节流器 |
| `geocoding` / `elevation` | ×1 | 按需（新增城市/海拔） | 结果本地缓存 |
| `marine` / `flood` | ×1 | 按需 | 仅对相关坐标（沿海 / 沿河）发起 |
| `satellite`（Himawari-9） | ×1 | 用户触发 | 不随主屏轮询 |
...
- **R-Q5**：**ERA5 滞后约 5 天**——"往年今日"落在近 5 天窗口时，取**最近可用日**并在 UI 标注实际日期，不得静默显示过期数据。
...
| **B1-2** | **短时降水卡（15 分钟降级版，中国标插值）**〔旧册已登记「降级做」，未落地〕 | **P0** | D5 / 0.0.6「分钟降水卡」/ 0.1.3「标明真实粒度，不伪造逐分钟」 | AC-B1-5 请求含 `minutely_15=precipitation,precipitation_probability`；AC-B1-6 主屏短时降水卡展示未来 2h 柱状序列 + 开始/停止时间 + 峰值雨势 + 30 分钟刻度；AC-B1-7 **中国及非原生覆盖区显式标注「15 分钟粒度 · 插值」**，禁止任何"逐分钟/雷达临近"措辞；AC-B1-8 无雨时显示完整两小时晴窗；**正在下雨时不显示晴窗**（沿用 0.0.8 纪律）；AC-B1-9 数据缺失**整卡隐藏**，不显示空槽 |
| **B1-3** | **历史 · 往年同日多年回看**（按年循环 + 节流） | **P1** | A3-2 / A3-3（未落地）+ D26 | AC-B1-10 逐年请求 `archive-api`，**`start_date`/`end_date` 显式必填**；近 5 年（P1）、近 10 年（P2）；AC-B1-11 年份循环**限并发 ≤3**，失败年份**静默跳过**（R-Q3）；AC-B1-12 **近 5 天窗口按 R-Q5 取最近可用日并标注实际日期**；AC-B1-13 每条记录标出具体年份；**缺有效温度的年份不堆叠「未知」占位**（沿用 0.1.5-beta3 纪律） |
| **B1-4** | **Himawari-9 卫星辐射**（GHI/DNI/DHI/GTI） | P2 | 原版无对应（**新增能力**） | AC-B1-14 接 `satellite-api.open-meteo.com/v1/archive`，`models=satellite_radiation_seamless`；AC-B1-15 辐照区块标注覆盖范围「Himawari-9 · 亚洲含中国 · 0.05° / 10 分钟 · 2015 起」；AC-B1-16 **非覆盖区隐藏该区块**；单测覆盖参数构造 + 4 字段解码 |
| **B1-5** | **洪水 + 海洋**（水文与海域） | **P1** | 原版无对应（**新增能力**） | AC-B1-17 洪水：`flood-api` 仅日粒度，取 `river_discharge` 及分位（`_mean/_median/_max/_min/_p25/_p75`）、`forecast_days 0–210`，**仅沿河坐标展示，否则隐藏**；AC-B1-18 海洋：`marine-api` **仅对沿海/海域坐标展示**；AC-B1-19 **两条链路独立 Result 收敛，失败互不影响、也不影响主屏天气**（沿用 A2-1 纪律） |
| **B1-6** | **集合概率 + 季节预报** | **P1** | 原版无对应（**新增能力**） | AC-B1-20 `ensemble`：`models` 必填、`forecast_days 0–35`，展示降水/温度**概率分位**；AC-B1-21 `seasonal`：EC46（46 天）+ SEAS5（7 个月），展示**趋势**并标注「季节尺度 · 非逐日预报」；AC-B1-22 **两者默认 OFF，仅用户主动开关后请求**（额度 ×4.0 / ×1.3，R-Q1）；AC-B1-23 文案统一用「概率 / 趋势」措辞，**不做确定性承诺** |
| **B1-7** | **台风路径（官方 nmc.cn，实验特性）** | P2 | D25 / 0.1.5-beta3「台风路径」 | AC-B1-24 接入 `typhoon.nmc.cn` `list_default` + `view_{longID}`，**实现 JSONP 双括号剥离的自定义解析器**；AC-B1-25 展示 120h 预报点 + 30KTS 风圈；AC-B1-26 **显式标注「实验特性 · 来源无 SLA · 不作为预警发布依据」**，来源不可用或资料超时效**显式提示**（沿用 0.1.5-beta3）；AC-B1-27 无台风活动时**整入口隐藏**；单测覆盖 JSONP 去包裹 + 空列表 + 字段缺失 |
...
**B1 交付物**：可安装 IPA（现有 CI macOS 流程不变）+ README 能力清单更新 + **单测增量 ≥ 25 用例**（遥测四字段、minutely_15 边界、Archive 按年循环与节流、卫星参数、洪水/海洋字段、ensemble/seasonal 参数、JSONP 剥离、displayCities 排序、时区渲染、缓存兜底）。

---

## 6. 批次二（B2）：自带 Key 增强层 —— 可选、默认 OFF

> **一句话**：把所有"免密做不到"的国内专有能力，收进一个**默认关闭**的设置区；用户填一把**和风免费 key**即解锁，不填则完全退回 B1 体验。
> **本批原则**：不填 key = 零配置完整可用；填 key = 增强，**失败即优雅回退，绝不阻塞主流程**。

| ID | 需求 | 优先级 | 原 Android 对应 | 验收标准（AC） |
|---|---|---|---|---|
| **B2-1** | **BYO-Key 设置区**（和风免费 key） | **P1** | D27「实验室接入，凭据只存本机」 | AC-B2-1 设置页新增「增强层（可选）」区，**默认 OFF**；AC-B2-2 填 key 后**仅存本机**（Keychain / 设备本地），**不进备份、不上传**，界面明示；AC-B2-3 提供**连接测试**按钮；AC-B2-4 key 无效/403 时明确提示，且**不阻塞免密体验**；AC-B2-5 未填 key 时该区仅显示说明，**主流程零配置可用**；AC-B2-6 源标注显示当前城市**实际使用的源** |
| **B2-2** | **中国分钟级 nowcast** | **P1** | D5 真·分钟级 / 0.1.3「短时降水优先显示开始/停止时间、峰值」 | AC-B2-7 启用后短时降水**升级为分钟级**（2h / 1km），粒度标注改为「分钟」；AC-B2-8 **未启用时自动回退 B1-2 的 15 分钟插值卡**；AC-B2-9 升级/回退切换时**不闪空、不混用两源时间点** |
...
| # | 风险 | 缓解 |
|---|---|---|
| R-1 | 免密端点「今天 200、明天 404」（无 SLA 官方源） | 台风（B1-7）等无 SLA 源**显式标注实验性**，不可用即**隐藏入口**，不阻断主屏 |
| R-2 | 免密额度被贵端点吃穿（×4.0 / ×1.3 / ×1844.5） | 执行 §4.3 硬规则 R-Q1（贵端点用户触发/默认 OFF）、R-Q2（加字段不加剧请求）、R-Q6（未算清倍数不上线） |
| R-3 | 按年回看并发过高 | R-Q3 限并发 ≤3 + 逐年串行 + 失败静默跳过 |
| R-4 | ERA5 滞后导致「往年今日」显示过期数据 | R-Q5 近 5 天窗口取**最近可用日**并标注实际日期 |
...
| # | 问题 | 影响 | 建议默认值 |
|---|---|---|---|
| Q1 | 免密短时降水卡在**中国以外**是否也显示（插值数据）？ | B1-2 UI 分支 | 显示，但插值标注随地区自动切换 |
| Q2 | B1-4 卫星辐射的入口形态（独立页 / 主屏区块）？ | B1-4 工作量 | 独立「辐照」区块，默认折叠 |
...
- **B1 免密拉满**（13 项）：遥测四字段 + 短时降水卡（标插值）+ 往年同日多年回看 + 卫星辐射 + 洪水海洋 + 集合/季节概率 + 官方台风（标实验）+ 纯文字 Tips + 开发者模式 + 更新说明页 + 街道定位 + 两处接线缺陷 + 离线兜底/熔断——**零凭据**。
- **B2 自带 Key 增强层**（6 项）：BYO-Key 设置区 + 中国分钟级 nowcast + 官方分级预警（四档着色）+ 官方生活指数 + 多源选择（QWeather 主 / Open-Meteo 补缺）+ 雷达观察位——**默认 OFF，失败即回退**。
- **B3 方向 B 收口**（6 项）：主题三档 + 氛围背景 12+ + 15 枚自绘图标与图标切换 + 横屏气象中枢 + 发牌式卡组 + 天气娘形象——**数据面收口之后**才动。

**不做**：小米源（结构性不可能）/ App Store·账号·云同步·埋点（产品红线）/ 社区·交流群·赞赏 / 长程气候预估（额度 ×1844.5）/ 彩云凭据 / 免密中国雷达与免密官方预警（档三不可达）。
