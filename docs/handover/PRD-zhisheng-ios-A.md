...
| A1-4 | **日出日落**（当日 + 显示在月相区） | P0 | D9a | AC-A1-11 请求含 `daily=sunrise,sunset`；AC-A1-12 月相区旁显示"日出 HH:mm · 日落 HH:mm"；AC-A1-13 ⚠️ sunrise/sunset 为 ISO 字符串时间，**必须与现有 unixtime 解码路径隔离**（新解码器 + 独立单测），不得混入 epoch 解析器 |
