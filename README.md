...
长按桌面小组件 → **编辑小组件** → 城市 → 选一个具体城市。

**为什么必须手动选**：默认的「跟随 App」依赖 App Group 共享容器，而
**重签侧载的产物上 App Group 不生效**，城市列表读不到，「跟随 App」就解析不出城市，
**小组件根本不会发起请求**——表现就是永远空着。选了具体城市后，小组件自力取数，正常显示。

如果你在小尺寸组件上看到一行「长按 → 编辑，选择城市」的提示，说的就是这件事。
