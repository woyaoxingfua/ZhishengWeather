...
///
/// 语义（B1-2 / AC-B1-7，事实更正·已实测验证）：数据为 **15 分钟粒度**；
/// 中国等非原生覆盖区为**逐小时插值到 15 分钟网格**（**非实况外推 / nowcast**）。
/// 领域层只承载数值，**不声明**其精度来源；「由逐小时插值·非实况外推」标注
/// 由 UI 文案承担（禁止暗示逐分钟 / 雷达临近）。
/// ⚠️ 数组长度**无法**区分原生与插值，绝不可据此推断数据质量。
