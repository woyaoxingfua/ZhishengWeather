...
//
//  F-C 语义收窄（R-C2）仍然成立，但**只约束生产路径**
//  （`WeatherProvider.makeResolution` → `WidgetDataResolver`）：
//  `payload` **仅当载荷属于本实例目标城市时才非 nil**
//  （旧：共享容器归属匹配；本轮：容器归属匹配或 L1 自力取数取回），
//  绝不拿其他城市的**真实数据**冒充（AC-C6 针对的是真实数据）。
//
//  ✅ 下发侧的**唯一**不变式（由 `WidgetDataResolver` 保证，单测逐条断言）：
//      **`payload != nil` ⟺ `emptyReason == nil`**
//  （有数据必无空因；有空因必无数据）。视图判空态一律看 `emptyReason`，
//  文案取自 `WidgetCopy` —— **不再**用「payload 非 nil ⟹ 必属本城」当判据
//  （该判据在生产路径成立，但对下面的预览占位不成立）。
//
//  ⚠️ 唯一不吃「属于本城」约束的是**画廊 / 预览占位**：本文件末尾的
//  `WidgetEntryResolution.placeholder`（`city == nil` + 示例 `payload`）。
//  它落在**「有载荷」那一侧**（`emptyReason == nil`），因此**满足**上面那条 `⟺`；
//  它只是不走生产路径，用的是显式标注的示例数据（WidgetKit 占位渲染的常规做法），
//  **不是**把别的城市的真实数据冒充成本城数据。
//
